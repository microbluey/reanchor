...
redistributed under the same licence as the code. They deliberately differ in
shape: prose with long sentences, a technical text dense with punctuation and
identifiers, a legal-style text with numbered clauses and repeated boilerplate,
and a CJK text where whitespace carries no word boundaries.

These are the same four documents as ``bench/documents.ts``, character for
character, so the two implementations are measured on the same corpus.
"""
...
DOCUMENTS: Final[tuple[str, ...]] = (PROSE, TECHNICAL, LEGAL, CJK)
